...
document.addEventListener('plusready', function() {
	...
	avatar = plus.storage.getItem('userspic');
	console.log(avatar);
...
function bindHtml(data) {
	...
	var html = '';
	minid = data[0].aid;
	console.log(minid); //用于更新
	maxid = data[data.length - 1].aid;
	console.log(maxid); //用于往上轮询
...
function findChatHistory() {
	...
	//	showWaiting('加载中...');
	$("#loadHistory").show();
	alert(uid + ',' + fid + ',' + maxid + ',' + page + ',' + ftype)
	$.ajax({
		type: "post",
...
function sendRed() {
	...
	$.ajax({
		...
		success: function(data) {
			...
			plus.nativeUI.closeWaiting();
			if(data > 0) {
				var html = '<div class="chat-me-hear" onclick="openRed(' + data + ')">' +
					'<div class="frien_img">' +
					'<img src="' + avatar + '"/>' +
					'</div>' +
