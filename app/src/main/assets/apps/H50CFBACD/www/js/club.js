	...
	$.ajax({
		...
		success: function(data) {
			...
			$.each(data, function(k, v) {
				...
				if(v.res!=null) {
					var on="clubchat('"+v.res.id+"','"+v.res.userid+"','"+v.res.name+"')";
...
function clubchat(id,uid,name){
	console.log(id + "," + name + "," + uid)
	create('club-chat.html',{fid: id, ftype: 1, fname: name, cludid: id, cuseid: uid });
}
				...
				$.each(data.info, function(key, vo) {
					...
					if(isnull) {
						output = '<ul class="mui-table-view news" data-aid="' + vo.fid + '" data-name="' + vo.name + '">' +
							...
							'</li>' +
							'</ul>';
						$(output).prependTo($('#team')); //列表不存在该聊天,头部插入
					}
