...
var a = / ^(^(\d{4}|\d{2})(\-|\/|\.)\d{1,2}\3\d{1,2}$)||(^\d{4}年\d{1,2}月\d{1,2}日$)$/; //日期格式
var xlh = /^[a-b0-9]{12}$/
	//获取头像
...
var isLogin = function(page) {
	...
	if(!uid) {
		toast('你还未登录');
		plus.webview.create('../view/login.html', 'login.html', {}, {
			spage: page
		}).show('slide-in-right');
	}
}
...
var showWating = function(info, time) {
	...
	var waiting = plus.nativeUI.showWaiting(info, {
		width: '80px',
		height: '80px',
...
function getcheck() {
	...
	$('#getdx').unbind('click');
	$('#getdx').css('background-color', 'gray');
	$('#getdx').text('正在发送..');
	$.ajax({
		type: "get",
		url: apiRoot + '?m=Home&c=user&a=xsecnd&phone='+tel,
		dataType: 'json',
		...
		error: function(e) {
			...
			$('#getdx').css('background-color', '#ff6700');
			$('#getdx').text('获取验证码');
		}
	})
}
