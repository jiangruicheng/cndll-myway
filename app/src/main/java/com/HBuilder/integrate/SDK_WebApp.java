...
import com.HBuilder.integrate.activity.BleService;
import com.HBuilder.integrate.eventtype.DisBleConn;
import com.HBuilder.integrate.eventtype.JSEvent;
import com.HBuilder.integrate.fragment.CarFragment;
...
public class SDK_WebApp extends Activity {
    ...
    private void init() {
        ...
        getFragmentManager().beginTransaction().replace(R.id.frame, carFragment).commit();
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if (keyCode == KeyEvent.KEYCODE_BACK) {
            SDK.obtainAllIWebview().get(SDK.obtainAllIWebview().size() - 1).evalJS("mui.back();");
            return true;
        }
