...
import com.HBuilder.integrate.activity.PathActivity;
...
public class CarStatuFragment extends Fragment {
    ...
    void oncheckout() {
        getActivity().getFragmentManager().beginTransaction().replace(R.id.fragment_frame, new CarCheckOutFragment()).addToBackStack("").commit();
    }
    ...
    void onupdata() {
        if (CarStatu.getDefault().getType().equals("SA")) {
            getActivity().getFragmentManager().beginTransaction().replace(R.id.fragment_frame, new UpdataFragment()).addToBackStack("").commit();
        } else {
