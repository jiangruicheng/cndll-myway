...
import com.cndll.myway.data.Updata;
import com.cndll.myway.data.Uuids;
import com.cndll.myway.eventtype.ConnSucc;
import com.cndll.myway.eventtype.DisBleConn;
import com.cndll.myway.eventtype.DisConnNotify;
import com.cndll.myway.eventtype.ReciveCmd;
import com.cndll.myway.eventtype.UpdataProg;
...
public class ConnBle {
    ...
    public ConnBle() {
        ...
        if (SendCmd == null) {
            SendCmd = RxBus.getDefault().toObservable(com.cndll.myway.eventtype.SendCmd.class).observeOn(AndroidSchedulers.mainThread()).subscribe(new Observer<com.cndll.myway.eventtype.SendCmd>() {
                @Override
                public void onCompleted() {
                ...
                @Override
                public void onNext(com.cndll.myway.eventtype.SendCmd sendCmd) {
                    try {
                        /*sendCmd.getCmd();*/
            ...
            if (next && !b) {
                ...
                try {
                    ...
                    if (i < Math.ceil((float) cont / 1024)) {
                        ...
                        RxBus.getDefault().post(new com.cndll.myway.eventtype.SendCmd().setCmd(new byte[]{0x04}));
                        i++;
                        return;
                    ...
                    new Thread() {
                        ...
                        public void run() {
                            ...
                            for (int i = 0; i < Math.ceil((float) finalUpdata.length / 20); i++) {
                                ...
                                RxBus.getDefault().post(new com.cndll.myway.eventtype.SendCmd().setCmd(a));
                                try {
                                    sleep(50);
                                } catch (InterruptedException e) {
                                    e.printStackTrace();
