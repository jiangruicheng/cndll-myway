...
public class Command {
    ...
    private static class CRC16Modbus {
        ...
        public synchronized static byte[] getchecksum(byte[] bytes) {
            ...
            if (bytes.length > 4 && bytes[0] == 0x4d && bytes[1] == 0x57) {
                ...
                Log.d("updatechesum", "getchecksum: " + Integer.toHexString(data[0]));
                Log.d("updatechesum", "getchecksum: " + Integer.toHexString(data[data.length - 1]));
                crc.update(data, 0, data.length);
            }
            /*for (byte d : bytes) {
                crc.update(d);
